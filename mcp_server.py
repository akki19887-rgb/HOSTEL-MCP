...
Suraksha - teen taale, teenon alag:
  1. RAASTA CHHUPA HUA HAI. URL me ek lamba gupt shabd hai (MCP_PATH_SECRET).
...
# MCP ka apna ek aur pehra hai: wo Host header dekhta hai, aur jo naam usne pehle se
# maan rakha ho bas wahi manzoor karta hai. Bina bataye ye sirf 'localhost' maanta hai,
# isliye Render par chadhte hi har request 421 me mar jati thi. Render khud apna naam
# RENDER_EXTERNAL_HOSTNAME me rakhta hai, to wahi utha lete hain - haath se kuch bharna
# nahi padta. Zarurat pade to MCP_ALLOWED_HOSTS se aur naam jode ja sakte hain.
_hosts: list[str] = []
for _h in os.environ.get("MCP_ALLOWED_HOSTS", "").split(","):
    _h = _h.strip()
    if _h:
        _hosts.append(_h)
...
if not _hosts:
    # Apne computer par test karte waqt.
    _hosts = ["localhost", "127.0.0.1"]
# Har naam ke saath uska port wala roop bhi, warna 'naam:10000' atak jata hai.
_ALLOWED_HOSTS = _hosts + [h + ":*" for h in _hosts]
print("Host header me ye naam manzoor hain:", ", ".join(_hosts))
...
mcp = FastMCP(
    ...
    transport_security=TransportSecuritySettings(
        ...
        allowed_origins=[],   # Origin naa ho to chalta hai; MCP client bhejta hi nahi.
    ),
)
